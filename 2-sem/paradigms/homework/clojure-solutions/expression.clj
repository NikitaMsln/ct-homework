...
(defn operator [op]
      (fn [& args] (fn [variables] (apply op (mapv (fn [x] (x variables)) args)))))
...
(defn meansqnum [& args]
      (/ (apply + (mapv (fn [x] (* x x)) args)) (count args)))
...
(definterface Expression (evaluate [variables]) (diff [variable]) (toStringInfix []))
...
(deftype OperatorConstructor [name evaluator d args]
         Expression
         (evaluate [this variables]
                   (apply evaluator (mapv (fn [element] (evaluate element variables)) args)))
         (diff [this variable] (apply d variable args))
         (toStringInfix [this] (cond
                                 (== 1 (count args)) (str "(" name " " (toStringInfix (first args)) ")")
                                 :else (str "(" (clojure.string/join (str " " name " ") (mapv toStringInfix args)) ")")))

         Object
         (toString [this]
                   (str "(" name " " (clojure.string/join " " (mapv toString args)) ")")))

(defn Operator [name evaluator d]
      (fn [& args] (OperatorConstructor. name evaluator d args)))

(def Add
  (Operator "+" +
            (fn [variable & args]
                (apply Add (mapv (fn [element] (diff element variable)) args)))))
(def Subtract
  (Operator "-" -
            (fn [variable & args]
                (apply Subtract (mapv (fn [element] (diff element variable)) args)))))
(def Multiply
  (Operator "*" *
            (fn [variable & args]
                (apply Add
                       (mapv
                         (fn [x] (apply Multiply (mapv (fn [y] (if (= x y) (diff y variable) y)) args))) args)))))
(def Negate
  (Operator "negate" - (fn [variable & args] (Negate (diff (first args) variable)))))
(def Divide
  (Operator "/" div
            (fn [variable & args]
                (if
                  ...
                  (apply Divide
                         (Subtract (apply Multiply (diff (first args) variable) (rest args))
                                   (Multiply (first args) (diff (apply Multiply (rest args)) variable)))
                         (into [] (concat (rest args) (rest args))))))))

(defn sumexp [& args] (apply + (mapv #(Math/exp %) args)))

(def Sumexp
  (Operator "sumexp" sumexp
            (fn [variable & args]
                (apply Add (mapv (fn [x] (Multiply (diff x variable) (Sumexp x))) args)))))
(def LSE
  (Operator "lse" (fn [& args] (Math/log (apply sumexp args)))
            (fn [variable & args]
                (let [value (apply Sumexp args)]
